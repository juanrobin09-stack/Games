...
- **Legible room exits**: every door is a built architectural passage — carved jambs, a lit lintel, a state-colored glow that spills further into the room as the player approaches, and barred geometry on locked doors so the state reads without relying on color — instead of a flat colored rectangle.
- **Legible room landmarks**: shop, rest, and event rooms each center on a dedicated lit structure — a merchant's stall, a brazier, or a mysterious shrine — so their interaction is always tied to something visible, not an invisible trigger zone in empty floor.
