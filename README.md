...
- **A floor with real depth, not a ruled grid**: each room's floor is tiled, once and cached, from irregularly-sized slabs — alternating rows offset like running-bond pavement so no straight seam lines up across the room — that each resample a small, randomly-positioned, often-mirrored window of one of two separate bands of a second reference photo's flagstone floor (`assets/textures/floor-and-shop.png`), get the same levels correction as the walls, and pick up the same zone tint. Sampling a small window from two separate source areas, instead of a single band nearly as large as each slab, is what keeps a room's floor from reading as the same few recognizable chunks of image tiled in a visible pattern.
