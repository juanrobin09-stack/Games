...
- **Procedural stone masonry**: walls are baked, per room, into staggered stone blocks with mortar grooves, cracks, sparse moss and distinct corner stones — seeded so every room looks unique — instead of a flat color fill, with doors carved directly into that same masonry as an irregular gap between worked jamb stones rather than a separate clean insert.
