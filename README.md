...
- **Real stone masonry, not a color fill**: walls are baked, per room, from hand-picked crops of an actual reference painting of ancient dungeon stone (`assets/textures/ancient-wall.png`) — resampled per block with a random offset and occasional mirroring, then hue-shifted to each zone's palette via canvas `'color'` blending so the photo's own depth and shading survive under three visually distinct zone tones — instead of a flat procedural fill, with doors carved directly into that same masonry as an irregular gap between worked jamb-and-torch stones (mirrored from the same source crop) rather than a separate clean insert.
- **A floor with real depth, not a ruled grid**: each room's floor is tiled, once and cached, from irregularly-sized slabs — alternating rows offset like running-bond pavement so no straight seam lines up across the room — that each resample a random window of a second reference photo's flagstone band (`assets/textures/floor-and-shop.png`) and pick up the same zone tint as the walls.
