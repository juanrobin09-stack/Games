...
The character sheet also shows a read-only **Ability Range** value next to Ability Damage: a base of **10**, scaling with the exact same `areaDamageMult` the four in-run range upgrades (§8) already apply — `getAbilityRangeDisplay(areaDamageMult) = round(10 × areaDamageMult × 10) / 10`, so it reads 10 → 10.5 → 11.55 → 13.28 → 15.54 across Keen Aim/Far Reach/Wide Blast/Eagle Eye exactly in step with the real mechanic. It's a presentational unit layered on top of the actual gameplay math (Ember Burst's radius is still `140px × areaDamageMult`, untouched) rather than a second formula — chosen because a literal reading of "base range 10" as the multiplier itself would have meant either a no-op or an 8×+ radius increase on Ember Burst, either of which would have broken existing ability balance for no reason.
...
|---|---|---|---|---|---|
| Ash Crawler | Chaser | 200 | 8 | 220 | Fast, hunts in numbers |
...
Zone 1's three starting enemies (Ash Crawler, Hollow, Flame Wisp) were leveled to a uniform 200 HP / 8 damage as part of the Level-30 rebalance — a deliberate simplification of what were previously three differentiated HP/damage tiers (16–22 HP, 6–9 damage) into one shared baseline for the player's very first fights, chosen over re-differentiating three new numbers per archetype since the brief specified a single Level-1 mob baseline; their behavior and role (fast chaser / slow tank / kiting ranged) are untouched, so the *feel* of each still differs even though the numbers don't.

Ash Crawler's own speed was separately nerfed from 255 to 220 in a later pass. At 255 (+34% over the player's 190 base move speed) it closed the ~110px burst-distance a dodge buys almost as fast as the dodge itself resolved, so a player who dodged had essentially no window before it was back in range — worse, since stamina only starts regenerating 0.55s after the last spend, the next dodge often wasn't affordable yet either, chaining into exactly the "no real options" pressure spiral the brief described. 220 (+16% over player base) keeps it a genuine chaser — still faster than walking away, so it always eventually closes on a player who isn't actively managing the fight — while cutting the *relative* closing speed while actively kiting after a dodge from 65px/s to 30px/s, which is the number that actually determines whether repositioning buys anything. Its HP, damage, telegraph, and attack range are untouched — the fix is speed only, per the brief's own instruction not to compensate by changing anything else (including player stamina).
...
| Warden's Bow | Ranged | 16 | 0.5s | 10 | Pierces 1 target, 420 range | Zone 3 elite room (in-run) |
...
The **Warden's Bow** is the run's one in-run weapon pickup: clearing Zone 3's (the Ember Citadel's) single guaranteed elite room grants it directly and auto-equips it, stacked on top of that room's normal upgrade-choice reward exactly the way the Drowned Sanctum's bonus stacks with its own room-clear roll (§3's "Upgrade Levels", §9). This corrects an earlier pass that tied it to Zone 2 instead — `Game.currentGateIds()` already anticipated moving it, since it merges a synthetic `'zone2'` id into the gate set once `run.zoneIndex >= 2` alongside the player's in-run `unlockedWeapons`, so the fix was a one-line change to the grant condition (`grantRoomClearReward`'s `room.type === 'elite' && run.zoneIndex === 2`) plus updating the character sheet's lock-reason copy — no gating mechanism needed to change. It reuses the same ranged-attack path Solar Spear already exercises (`kind:'ranged'`, a pierce-1 projectile), so nothing new had to be built for it to fire correctly. Attack Speed's three upgrades — Brisk Hands (+3%), Shadow Step (+6%), Quickdraw (+9%) — are gated `requiresUnlock:'bow'` and only enter the pool once the Bow is owned (`Game.currentGateIds()` merges the player's in-run `unlockedWeapons` into the same gate-id set `requiresUnlock` already checks for permanent Soul-Ash unlocks and zone-progress — see §8). The character sheet shows the lock explicitly as "unlocked at dungeon Level 3" rather than naming a specific enemy or boss, since the exact in-Level-3 encounter design is left open for whenever that zone's own content gets a dedicated pass.
...
**The return trip — the same stairwell, in reverse.** The arrival stairwell isn't only scenery: standing at it in any zone but the first offers *Ascend to* the previous zone, walking the exact same scripted transition backward (`Game.beginAscent`/`completeAscent`, `RunState.retreatZone`) and landing at the mouth of the previous zone's own `stairsDown` — the same well the player originally descended, arriving from below this time. Nothing about either zone's state is touched by the trip: `RunState.layouts` generates and keeps every zone's full room graph in memory for the whole run (not just the current one), so a Room object's `visited`/`cleared`/`chest`/`enemies` fields are the same live objects whichever direction the player is currently facing — retreating and re-descending is just changing which already-existing layout `zoneIndex` currently points at, never a regeneration or a reset. Retreating always lands in the previous zone's heart/boss room specifically (where its own down-stairs physically are) rather than its start room, so the round trip reads as retracing real space. The interaction only ever appears where a `stairsUp` obstacle exists at all (`zoneIndex > 0`, the same condition that already governed whether one gets placed) and only inside a zone's own start room — which nothing lets the player reach while a room-clear gate (a heart fight, the Sanctum's sealed rite, the boss room) is still open, so a retreat can never orphan an in-progress encounter.
...
`HUD.refreshMinimap` renders the current zone's room graph (top-right, `hud-top-right`) as a double-resolution CSS grid: even track indices are room cells, odd ones are thin connector bars in the gaps between door-linked rooms, so corridors are real grid cells rather than absolutely-positioned overlays laid on top. A room is drawn once it's been **visited**, is the **current** room, or — the one deliberate exception — is the zone's **heart/boss room**, which is always drawn from the moment the zone is entered, whatever the player has or hasn't explored, with its own unmistakable marker (an oversized, rotated, gold-gradient diamond with a permanent pulse, reusing the existing `legendaryPulse` animation) so there's always one stable, identifiable "this is where the run is headed" landmark. Every other room stays hidden until reached — except a room adjacent to an already-visited one, which gets a faint outline-only "known to exist, not yet entered" hint pip instead of staying fully blank, so the map grows legibly as doors are found rather than jumping from nothing straight to fully revealed. A connector is only ever drawn between two rooms that are both actually shown, so a corridor line can never itself leak what's behind an unexplored door. The current room gets its own ember-glow ring on top of whatever else applies (including the destination marker, if the two coincide). The widget's cell/gap sizes are fixed (18px/8px) for crisp, legible rendering at any zone size, then the whole thing is measured and uniformly scaled down (`transform: scale()`, anchored top-right to match its corner) if a large zone's natural footprint would otherwise exceed a small fixed cap (176px) — so it stays a genuine *mini*map rather than growing with the dungeon, while never scaling up past its native size for small zones. Below 640px viewport width it's hidden entirely, same as before this pass — that threshold is shared with the touch-control layout, which already claims that screen space.

The heart/boss room's marker is stable in *identity* every run — same shape, same color, same behavior, always shown — rather than at a literal fixed pixel position: since it's always generated as the single farthest room from the entrance (a spanning-tree leaf, §10), its position in the grid necessarily varies with the room graph's shape, and forcing a literal fixed screen coordinate for it would mean either drawing it disconnected from its own connector lines (incoherent) or distorting the rest of the map to compensate (worse legibility for everything else). This is a considered reading of "stable, identifiable destination," not a limitation — flagged here in case a literal fixed-coordinate marker (e.g. a corner compass icon, separate from the room graph entirely) turns out to be what's actually wanted instead.
...
- **The Level-30 rebalancing pass** narrowed Zone 1 to one deliberately simple baseline (100 starting / 180 ceiling HP, 200 HP / 8 damage mobs, 17 fixed M1 damage) specifically so early runs read as *legible* rather than swingy, then reintroduces power gradually and gates two stats — Attack Speed, Ability Damage — behind concrete gameplay moments (finding the Bow; reaching Zone 2 for Ability Damage) instead of making them available from the first stat point. The Player Level cap of 30 exists so the stat-point curve has a defined endpoint to design future zones' XP/enemy scaling against, rather than growing unbounded.
- **A follow-up correction pass** fixed the starting HP figure above (an earlier pass had it at 130, not the intended 100 — the 180 ceiling was always correct and unchanged), moved the Bow's unlock from Zone 2 to Zone 3 (Attack Speed follows it automatically, since it's gated on Bow ownership rather than a zone index directly), and implemented the ability-range display value ("base 10") the previous pass had flagged as ambiguous and left out — see §3's "Player Level & Stat Points" for how it's derived without touching the underlying `areaDamageMult` mechanic. The same pass added the minimap rework (§10) and bidirectional zone stairs (§9) — both purely additive UI/navigation features that don't change any combat or economy number.
