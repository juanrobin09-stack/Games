import { floorAssetImage } from '@/rendering/FloorAsset';

/**
 * The shop half of the same reference sheet FloorAsset.ts loads is a sprite
 * sheet of individual props painted on a flat near-black background rather
 * than a photo with real transparency. Since the source has no alpha
 * channel, each sprite crop is chroma-keyed once — near-black pixels erased
 * to transparent, feathered rather than hard-cut so the silhouette's edge
 * doesn't look jagged — and the processed result cached, so every
 * subsequent draw is a normal, cheap drawImage.
 */
