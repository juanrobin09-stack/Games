...
export class UpgradeSelectUI {
  ...
  constructor(container: HTMLElement, choices: UpgradeDefinition[], callbacks: UpgradeSelectCallbacks) {
    const cards = choices.map((def) => {
      const button = el(
        ...
        [
          ...
          el('div', { class: 'card-tags' }, [el('span', { class: `tag rarity-${def.rarity}` }, [def.rarity])]),
        ]
      );
      return button;
    });
    ...
    const panel = el('div', { class: 'screen-panel wide panel pop-in' }, [
      el('div', { class: 'screen-title' }, ['A Blessing Awaits']),
      el('div', { class: 'screen-subtitle' }, ['Choose one. The Ember remembers every choice.']),
