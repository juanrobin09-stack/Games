...
### The brazier's glow, reprojected and re-lit against the same full-scene reference

Two follow-ups from that same character-and-torch reference.

First, tried reprojecting `BRAZIER_RING_TEXTURE`'s own already-solved
alpha shape onto this image's torch — a geometric answer to the
problem the very first two brazier rounds' floor-diff technique
solved differently: this scene's torch sits on a busy, unevenly-lit
floor with no clean background to key and no exact-pixel match to this
project's own `floor_stone.png` to diff against (a different, non-
square canvas — nothing here was generated by compositing over this
project's own floor tile the way the first two rounds' references
were), and its ring/gem colors overlap the lit floor's own too closely
for a brightness or saturation threshold to separate them. But the
ring's own geometry is already known and symmetric: located its 4
cardinal gems by brightness (a reliable signal even where the ring
metal itself isn't), used the two opposite pairs' spacing to solve for
this image's center and scale relative to the same measurement already
made on `brazier_ring.png`, then resampled this image's own pixels
through the existing alpha mask at that computed scale/offset. The
result aligned convincingly — but is a single static flame pose
baked into the ring, from a harder source, not an improvement on the
already-integrated ring + 8-frame animation. Not used; kept only as
confirmation the design is pixel-for-pixel the one already shipped.

Second, and the one actually worth keeping: side by side, that same
reference's torch sits in a much richer, wider pool of warm light than
this project's own in-game render did — the design was never the
question, the light was. `_update_light()`'s shared "every other lit
prop" branch gave `BRAZIER` the same `light_radius`/`intensity` as
`SHRINE`/`SARCOPHAGUS`/every other glowing prop that's meant to read as
one light source among several in a room. A floor brazier reads
differently — closer to the room's actual light — so it now gets its
