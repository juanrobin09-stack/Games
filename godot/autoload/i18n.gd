extends Node
## Autoload: I18n
## Ports i18n/index.ts's t()/tc() + i18n/fr.ts's FR_CONTENT/FR_UI verbatim —
## every string below is the source's own French text, not a fresh
## translation. "Ember" stays untranslated throughout, deliberately (the
## source's own header comment): it's the game's central lore proper noun,
## not an ordinary word, the same way an invented in-fiction term usually
## stays put across a localization. "Cinder" (an ordinary word, unlike
## "Ember") is translated normally as "Cendre".
##
## Not reactive, matching the source's own explicit non-goal: text there is
## baked into DOM nodes at construction time, so a language change needs a
## full page reload to retranslate anything already on screen — TS's
## SettingsMenu does exactly that. This port's own screens are already
## built fresh every time they're shown (MainMenuUI.show_main_menu() and
## every other screen's own static show_*() factory), so a language change
## takes effect the next time each screen is (re)opened, the same
## practical effect as the source's reload without needing one.
##
## Wired into MainMenuUI so far (the first screen a language change is
## actually visible on) — the rest of this port's UI (HUD, pause menu,
## shop/event/upgrade screens, inventory, armory, victory/defeat, settings
## itself) still builds its own strings as hardcoded English. Both
## dictionaries below are already complete for when each of those gets its
## own pass; wiring every remaining call site is a separate, large,
## mechanical sweep, not attempted in this one.

func locale() -> String:
	return MetaProgression.settings.get("language", "fr")
...
## documentation of what the key means) — exact port of t(key, fallbackEn).
func t(key: String, fallback_en: String) -> String:
	if locale() == "en":
		return fallback_en
	return FR_UI.get(key, fallback_en)

## Data-driven content (enemies/weapons/abilities/upgrades/permanent
## upgrades/zones/unlocks/synergies/world events & their options) — looked
## up by the object's own stable id plus a field name ("name",
## "description", "subtitle", "title", "label", "detail"...), falling back
## to the English value already sitting in the resource — exact port of
## tc(id, field, fallbackEn).
func tc(id: String, field: String, fallback_en: String) -> String:
	if locale() == "en":
		return fallback_en
	var entry: Dictionary = FR_CONTENT.get(id, {})
	return entry.get(field, fallback_en)

const FR_CONTENT: Dictionary = {
	# ---------- Enemies ----------
	...
	"blightbloat": {
		...
		"description": "Un Creux si gorgé des spores des ruines que son corps est devenu un sac gonflé et luminescent. Il veut être près de vous quand il éclate.",
	},
	"hollowWarden": {
		"name": "Gardien Creux",
		"description": "Les derniers soldats du guet en ruines. Son bouclier est la porte qu'il est mort à garder — rien ne passe par l'avant. Contournez-le, ou punissez sa charge.",
	},
	"sunkenWarden": {
	...
	"bow": {
		"name": "Arc du Gardien",
		"description": "Récupéré sur un Gardien déchu des Ruines Noyées. Rapide là où la lame est lente — sa véritable portée se gagne à l'usage.",
	},
	...
	"keen-aim": {"name": "Visée Affûtée", "description": "+5% portée de compétence."},
	"brisk-hands": {"name": "Mains Vives", "description": "+3% vitesse d'attaque."},
	"far-reach": {"name": "Longue Portée", "description": "+10% portée de compétence."},
	...
	"wide-blast": {"name": "Explosion Large", "description": "+15% portée de compétence."},
	...
	"shadow-step": {"name": "Pas de l'Ombre", "description": "+6% vitesse d'attaque, silencieux comme l'obscurité."},
	"eagle-eye": {"name": "Œil de Faucon", "description": "+17% portée de compétence."},
	"quickdraw": {"name": "Tir Rapide", "description": "+9% vitesse d'attaque."},
	...
	"wardensOath": {
		...
		"description": "Un gardien de pierre s'agenouille devant un bouclier fissuré, ses lèvres encore figées sur le dernier mot d'un serment. « Tenez la ligne. »",
	},
	...
	"sporeMother": {
		...
		"description": "Une masse fongique pulsante emplit l'alcôve, respirant lentement. Ce dont elle a poussé est encore là-dedans, et ce n'est pas tout à fait mort.",
	},
...
const FR_UI: Dictionary = {
	...
	"banner.bowFound": "L'ARC DU GARDIEN",
	"toast.bowFound": "Une arme rapide là où la lame est lente. La vitesse d'attaque a enfin de quoi s'exprimer.",
